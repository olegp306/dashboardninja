...
export function AgentTimeline({ agent, logs, skin }: { agent: Agent; logs: AgentLog[]; skin: DashboardSkin }) {
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between">
        <h3 className={["text-sm font-semibold", theme.textPrimary].join(" ")}>Activity timeline</h3>
        <span className={["text-[11px]", theme.textMuted].join(" ")}>{items.length} events</span>
      </div>
      <div className={["mt-1 text-[11px]", theme.textMuted].join(" ")}>Source: agent {agent.name}</div>

      <div className="mt-3 space-y-2">
        {items.length === 0 ? (
          <p className={["text-xs", theme.textMuted].join(" ")}>No activity yet.</p>
        ) : (
          items.map((log) => (
            <div key={log.id} className={["rounded-lg border p-3", theme.border, theme.panelMuted].join(" ")}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className={["text-xs", theme.textSecondary].join(" ")}>{log.message}</p>
                  {log.output ? <p className={["mt-1 text-[11px] break-words", theme.textMuted].join(" ")}>{log.output}</p> : null}
                </div>
                <div className="shrink-0 text-right">
                  ...
                  <div className={["mt-1 text-[11px]", theme.textMuted].join(" ")}>{timeAgo(log.createdAt)}</div>
                </div>
              </div>
            </div>
          ))
